...
# Read-only D2K preflash gate for RM11 Pro / NX809J.
# D2K keeps D2J's scoped Android 16 service compatibility, but removes the
...
EXPECTED_FINGERPRINT_PART="${EXPECTED_FINGERPRINT_PART:-orangefox_NX809J_codingbr_d2k}"
EXPECTED_MARKER="${EXPECTED_MARKER:-ro.rm11.decrypt_candidate_d2k=d2k-scoped-a16-binder-vintf-manual-keystore}"
...
PRODUCT="orangefox_NX809J_codingbr_d2k"
...
need_file "${DEVICE_DIR}/orangefox_NX809J_codingbr_d2k.mk"
need_file "${DEVICE_DIR}/d2k/recovery/root/init.recovery.qcom.rc"
need_file "$TWRP_CPP"

[ ! -e "$UNPACK_DIR" ] || fail "unpack directory already exists: $UNPACK_DIR"

printf '===== D2K PREFLASH VERIFY =====\n'
printf 'image: %s\n' "$IMAGE"
printf 'fox_dir: %s\n' "$FOX_DIR"
printf 'unpack_dir: %s\n\n' "$UNPACK_DIR"
...
check_sha "$IMAGE" "$EXPECTED_SHA256" "frozen D2K image"
...
contains_literal "${DEVICE_DIR}/AndroidProducts.mk" '$(LOCAL_DIR)/orangefox_NX809J_codingbr_d2k.mk' "PRODUCT_MAKEFILES D2K entry"
contains_literal "${DEVICE_DIR}/AndroidProducts.mk" 'orangefox_NX809J_codingbr_d2k-ap2a-eng' "COMMON_LUNCH_CHOICES D2K eng entry"
contains_literal "${DEVICE_DIR}/AndroidProducts.mk" 'orangefox_NX809J_codingbr_d2k-ap2a-userdebug' "COMMON_LUNCH_CHOICES D2K userdebug entry"

if grep -E '^RM11_NO_DECRYPT_PRODUCTS.*orangefox_NX809J_codingbr_d2k' "${DEVICE_DIR}/BoardConfig.mk" >/dev/null; then
  fail "${PRODUCT} is listed in RM11_NO_DECRYPT_PRODUCTS"
fi
pass "${PRODUCT} is not in RM11_NO_DECRYPT_PRODUCTS"

d2k_block="$(sed -n '/ifeq ($(TARGET_PRODUCT),orangefox_NX809J_codingbr_d2k)/,/endif/p' "${DEVICE_DIR}/BoardConfig.mk")"
printf '%s\n' "$d2k_block" | grep -Fq '$(DEVICE_PATH)/d2i' || fail "D2K TARGET_RECOVERY_DEVICE_DIRS block does not include \$(DEVICE_PATH)/d2i"
printf '%s\n' "$d2k_block" | grep -Fq '$(DEVICE_PATH)/d2k' || fail "D2K TARGET_RECOVERY_DEVICE_DIRS block does not include \$(DEVICE_PATH)/d2k"
if printf '%s\n' "$d2k_block" | grep -Fq '$(DEVICE_PATH)/d2h'; then
  fail "D2K TARGET_RECOVERY_DEVICE_DIRS includes D2H"
fi
pass "D2K TARGET_RECOVERY_DEVICE_DIRS includes d2i+d2k and excludes d2h"

if [ -e "${DEVICE_DIR}/d2k/recovery/root/system/lib64/libbinder_ndk.so" ] || [ -e "${DEVICE_DIR}/d2k/recovery/root/system/lib64/libvintf.so" ]; then
  fail "D2K source overlay contains global system lib64 Binder/VINTF replacement"
fi
pass "D2K source overlay has no global system lib64 Binder/VINTF replacement"

contains_literal "${DEVICE_DIR}/orangefox_NX809J_codingbr_d2k.mk" "$EXPECTED_MARKER" "D2K source property marker"
contains_literal "${DEVICE_DIR}/d2k/recovery/root/init.recovery.qcom.rc" 'setprop prepdecrypt.setpatch true' "D2K prepdecrypt.setpatch restore"
contains_literal "${DEVICE_DIR}/d2k/recovery/root/init.recovery.qcom.rc" 'start prepdecrypt.vendor' "D2K prepdecrypt.vendor trigger restore"
if grep -R -Fq 'interface aidl android.system.keystore2.IKeystoreService/default' "${DEVICE_DIR}/d2k/recovery/root" 2>/dev/null; then
  fail "D2K source overlay contains keystore2 lazy AIDL interface"
fi
pass "D2K source overlay omits keystore2 lazy AIDL interface"
...
contains_literal "${ROOT_DIR}/init.recovery.qcom.rc" 'setprop prepdecrypt.setpatch true' "unpacked D2K prepdecrypt.setpatch restore"
contains_literal "${ROOT_DIR}/init.recovery.qcom.rc" 'start prepdecrypt.vendor' "unpacked D2K prepdecrypt.vendor trigger restore"

for trigger in \
  'sys.rm11.d2k.start_qseecomd' \
  'sys.rm11.d2k.start_keymint_qti' \
  'sys.rm11.d2k.start_onekeymint' \
  'sys.rm11.d2k.start_gatekeeper' \
  'sys.rm11.d2k.start_weaver' \
  'sys.rm11.d2k.start_secure_element' \
  'sys.rm11.d2k.start_keystore2'
do
...
if grep -R -Fq 'sys.rm11.d2h.start_' "$ROOT_DIR" 2>/dev/null; then
  fail "unpacked D2K ramdisk contains D2H trigger names"
fi
pass "unpacked D2K ramdisk does not contain D2H trigger names"

if grep -R -Fq 'sys.rm11.d2j.start_' "$ROOT_DIR" 2>/dev/null; then
  fail "unpacked D2K ramdisk contains D2J trigger names"
fi
pass "unpacked D2K ramdisk does not contain D2J trigger names"

if [ -e "${ROOT_DIR}/init.recovery.wlan.rc" ]; then
  fail "D2K ramdisk unexpectedly contains init.recovery.wlan.rc"
fi
pass "D2K ramdisk does not contain init.recovery.wlan.rc"

if grep -R -Fq 'import /init.recovery.wlan.rc' "${ROOT_DIR}/init.recovery.qcom.rc" 2>/dev/null; then
  fail "D2K init.recovery.qcom.rc imports Wi-Fi lane"
fi
pass "D2K init.recovery.qcom.rc does not import Wi-Fi lane"

need_file "${ROOT_DIR}/system/etc/init/keystore2.rc"
if grep -R -Fq 'interface aidl android.system.keystore2.IKeystoreService/default' "${ROOT_DIR}/system/etc/init/keystore2.rc" 2>/dev/null; then
  fail "unpacked D2K keystore2 exposes lazy AIDL interface"
fi
pass "unpacked D2K keystore2 omits lazy AIDL interface"
...
do
  ...
  contains_literal "$rc" '/vendor/lib64/rm11-d2k-compat' "$(basename "$rc") D2K compat LD_LIBRARY_PATH"
done
...
[ "$ramdisk_binder_sha" != "$TWRP_BINDER_NDK_SHA256" ] || fail "unpacked D2K global libbinder_ndk.so matches D2H/TWRP replacement"
[ "$ramdisk_binder_sha" != "$STOCK_A16_BINDER_NDK_SHA256" ] || fail "unpacked D2K global libbinder_ndk.so was replaced by stock Android 16"
pass "unpacked D2K global libbinder_ndk.so is unchanged from D2I-style recovery userspace"

ramdisk_vintf_sha="$(sha256sum "${ROOT_DIR}/system/lib64/libvintf.so" | awk '{print $1}')"
[ "$ramdisk_vintf_sha" != "$TWRP_LIBVINTF_SHA256" ] || fail "unpacked D2K global libvintf.so matches D2H/TWRP replacement"
pass "unpacked D2K global libvintf.so is not the D2H/TWRP replacement"

COMPAT_DIR="${ROOT_DIR}/vendor/lib64/rm11-d2k-compat"
need_dir "$COMPAT_DIR"
...
pass "crypto expectation is provable: D2K is outside RM11_NO_DECRYPT_PRODUCTS, so BoardConfig selects the TW_INCLUDE_CRYPTO lane"

printf '\n===== AVB FINGERPRINT =====\n'
grep -F 'com.android.build.recovery.fingerprint' "$tmp_avb" || true

printf '\nRESULT: PASS. D2K preflash gate passed. This script does not flash anything.\n'
