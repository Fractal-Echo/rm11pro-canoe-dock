...
# Read-only D2N preflash gate for RM11 Pro / NX809J.
...
IMAGE="${IMAGE:-${ARTIFACT_DIR}/recovery.img}"
...
EXPECTED_FINGERPRINT_PART="${EXPECTED_FINGERPRINT_PART:-orangefox_NX809J_codingbr_d2n}"
EXPECTED_MARKER="${EXPECTED_MARKER:-ro.rm11.decrypt_candidate_d2n=d2n-auto-decrypt-ui-gatekeeper-polish}"
...
PRODUCT="orangefox_NX809J_codingbr_d2n"
...
check_service_semantics() {
  ...
  assert_rc_has "${root}/vendor/etc/init/android.hardware.gatekeeper-service-qti.rc" 'on property:vendor.sys.listeners.registered=true' "${label} gatekeeper listener trigger"
  assert_rc_has "${root}/vendor/etc/init/android.hardware.gatekeeper-service-qti.rc" 'start vendor.gatekeeper_default' "${label} gatekeeper listener start"
  ...
  assert_rc_has "${root}/vendor/etc/init/android.hardware.gatekeeper-service-qti.rc" 'setenv LD_LIBRARY_PATH /vendor/lib64/rm11-d2n-libcxx:/vendor/lib64:/vendor/lib64/hw:/vendor/lib:/vendor/lib/hw:/system/lib64:/system/lib' "${label} gatekeeper libc++ LD path"
  ...
  assert_rc_has "${root}/system/etc/init/keystore2.rc" 'setenv LD_LIBRARY_PATH /vendor/lib64/rm11-d2n-libcxx:/system/lib64:/vendor/lib64:/system/lib:/vendor/lib' "${label} keystore2 libc++ LD path"
  ...
  assert_rc_has "${root}/vendor/etc/init/android.hardware.boot-service.qti.rc" 'setenv LD_LIBRARY_PATH /vendor/lib64/rm11-d2n-libcxx:/vendor/lib64:/vendor/lib64/hw:/vendor/lib:/vendor/lib/hw:/system/lib64:/system/lib' "${label} boot HAL libc++ LD path"
...
check_ui_defaults() {
  local root="$1"
  local label="$2"
  local script="${root}/system/bin/rm11-d2n-ui-defaults.sh"

  assert_rc_has "${root}/init.recovery.qcom.rc" 'on property:init.svc.recovery=running' "${label} UI defaults recovery trigger"
  assert_rc_has "${root}/init.recovery.qcom.rc" 'start rm11-d2n-ui-defaults' "${label} UI defaults service start"
  assert_rc_has "${root}/init.recovery.qcom.rc" 'service rm11-d2n-ui-defaults /system/bin/rm11-d2n-ui-defaults.sh' "${label} UI defaults service declaration"
  assert_rc_has "${root}/init.recovery.qcom.rc" 'class late_start' "${label} UI defaults late_start class"
  assert_rc_has "${root}/init.recovery.qcom.rc" 'disabled' "${label} UI defaults starts disabled"
  assert_rc_has "${root}/init.recovery.qcom.rc" 'oneshot' "${label} UI defaults oneshot"

  need_file "$script"
  [ -x "$script" ] || fail "${label} UI defaults script is not executable: ${script}"
  pass "${label} UI defaults script is executable"
  contains_literal "$script" '/system/bin/twrp set tw_screen_timeout_secs 0' "${label} tw_screen_timeout_secs default"
  contains_literal "$script" '/system/bin/twrp set tw_no_screen_timeout 1' "${label} tw_no_screen_timeout default"
  contains_literal "$script" '/system/bin/twrp set tw_screen_timeout_temp 0' "${label} tw_screen_timeout_temp default"
  contains_literal "$script" '/system/bin/twrp set lock_btn 1' "${label} lock_btn default"
  contains_literal "$script" '/system/bin/twrp set lock_action 0' "${label} lock_action default"
}
...
need_file "${DEVICE_DIR}/orangefox_NX809J_codingbr_d2n.mk"
need_file "${DEVICE_DIR}/d2n/recovery/root/init.recovery.qcom.rc"
need_file "$TWRP_CPP"

[ ! -e "$UNPACK_DIR" ] || fail "unpack directory already exists: $UNPACK_DIR"

printf '===== D2N PREFLASH VERIFY =====\n'
printf 'image: %s\n' "$IMAGE"
printf 'fox_dir: %s\n' "$FOX_DIR"
printf 'unpack_dir: %s\n\n' "$UNPACK_DIR"
...
check_sha "$IMAGE" "$EXPECTED_SHA256" "frozen D2N image"
...
contains_literal "${DEVICE_DIR}/AndroidProducts.mk" '$(LOCAL_DIR)/orangefox_NX809J_codingbr_d2n.mk' "PRODUCT_MAKEFILES D2N entry"
contains_literal "${DEVICE_DIR}/AndroidProducts.mk" 'orangefox_NX809J_codingbr_d2n-ap2a-eng' "COMMON_LUNCH_CHOICES D2N eng entry"
contains_literal "${DEVICE_DIR}/AndroidProducts.mk" 'orangefox_NX809J_codingbr_d2n-ap2a-userdebug' "COMMON_LUNCH_CHOICES D2N userdebug entry"

if grep -E '^RM11_NO_DECRYPT_PRODUCTS.*orangefox_NX809J_codingbr_d2n' "${DEVICE_DIR}/BoardConfig.mk" >/dev/null; then
  fail "${PRODUCT} is listed in RM11_NO_DECRYPT_PRODUCTS"
fi
pass "${PRODUCT} is not in RM11_NO_DECRYPT_PRODUCTS"

d2n_block="$(sed -n '/ifeq ($(TARGET_PRODUCT),orangefox_NX809J_codingbr_d2n)/,/endif/p' "${DEVICE_DIR}/BoardConfig.mk")"
[ -n "$d2n_block" ] || fail "missing D2N TARGET_PRODUCT block in BoardConfig.mk"
for overlay in d2i d2k d2l d2m d2n; do
  printf '%s\n' "$d2n_block" | grep -Fq "\$(DEVICE_PATH)/${overlay}" || fail "D2N TARGET_RECOVERY_DEVICE_DIRS block does not include \$(DEVICE_PATH)/${overlay}"
done
if printf '%s\n' "$d2n_block" | grep -Fq '$(DEVICE_PATH)/d2h'; then
  fail "D2N TARGET_RECOVERY_DEVICE_DIRS includes D2H"
fi
pass "D2N TARGET_RECOVERY_DEVICE_DIRS includes d2i+d2k+d2l+d2m+d2n and excludes d2h"
printf '%s\n' "$d2n_block" | grep -Fq 'TW_NO_SCREEN_TIMEOUT := true' || fail "D2N BoardConfig block does not enable TW_NO_SCREEN_TIMEOUT"
pass "D2N BoardConfig enables TW_NO_SCREEN_TIMEOUT"
printf '%s\n' "$d2n_block" | grep -Fq 'OF_USE_LOCKSCREEN_BUTTON := true' || fail "D2N BoardConfig block does not enable OF_USE_LOCKSCREEN_BUTTON"
pass "D2N BoardConfig enables OF_USE_LOCKSCREEN_BUTTON"
printf '%s\n' "$d2n_block" | grep -Fq 'TW_INPUT_BLACKLIST :=' || fail "D2N BoardConfig block does not preserve touch blacklist"
pass "D2N BoardConfig preserves touch blacklist"

SRC_ROOT="${DEVICE_DIR}/d2n/recovery/root"
reject_literal "$SRC_ROOT" 'sys.rm11.d2m.start_' "D2M trigger"
...
reject_literal "$SRC_ROOT" 'ro.rm11.decrypt_candidate_d2m' "D2M marker"
...
reject_literal "$SRC_ROOT" 'rm11-d2m-libcxx' "D2M libc++ reference"
reject_literal "$SRC_ROOT" 'import /init.recovery.wlan.rc' "Wi-Fi import"
[ ! -e "${SRC_ROOT}/init.recovery.wlan.rc" ] || fail "D2N source unexpectedly contains init.recovery.wlan.rc"
pass "D2N source has no init.recovery.wlan.rc"
[ ! -e "${SRC_ROOT}/vendor/lib64/rm11-d2m-libcxx" ] || fail "D2N source unexpectedly contains rm11-d2m-libcxx directory"
pass "D2N source has no rm11-d2m-libcxx directory"

contains_literal "${DEVICE_DIR}/orangefox_NX809J_codingbr_d2n.mk" "$EXPECTED_MARKER" "D2N source property marker"
contains_literal "${SRC_ROOT}/init.recovery.qcom.rc" 'setprop prepdecrypt.setpatch true' "D2N source prepdecrypt.setpatch restore"
contains_literal "${SRC_ROOT}/init.recovery.qcom.rc" 'start prepdecrypt.vendor' "D2N source prepdecrypt.vendor trigger restore"

for trigger in \
  'sys.rm11.d2n.start_qseecomd' \
  'sys.rm11.d2n.start_keymint_qti' \
  'sys.rm11.d2n.start_onekeymint' \
  'sys.rm11.d2n.start_gatekeeper' \
  'sys.rm11.d2n.start_weaver' \
  'sys.rm11.d2n.start_secure_element' \
  'sys.rm11.d2n.start_keystore2'
do
  grep -R -Fq "$trigger" "$SRC_ROOT" 2>/dev/null || fail "D2N source is missing trigger ${trigger}"
  pass "D2N source contains ${trigger}"
done

check_sha "${SRC_ROOT}/system/bin/keystore2" "$EXPECTED_KEYSTORE2_SHA256" "D2N source TWRP keystore2"
check_sha "${SRC_ROOT}/vendor/lib64/rm11-d2n-libcxx/libc++.so" "$EXPECTED_LIBCXX_SHA256" "D2N source scoped libc++"
...
check_service_semantics "$SRC_ROOT" "D2N source"
check_ui_defaults "$SRC_ROOT" "D2N source"
...
contains_literal "${ROOT_DIR}/init.recovery.qcom.rc" 'setprop prepdecrypt.setpatch true' "unpacked D2N prepdecrypt.setpatch restore"
contains_literal "${ROOT_DIR}/init.recovery.qcom.rc" 'start prepdecrypt.vendor' "unpacked D2N prepdecrypt.vendor trigger restore"
reject_literal "$ROOT_DIR" 'sys.rm11.d2m.start_' "unpacked D2M trigger"
...
reject_literal "$ROOT_DIR" 'ro.rm11.decrypt_candidate_d2m' "unpacked D2M marker"
...
if grep -R --include='*.rc' --include='default.prop' --include='prop.default' -Fq 'rm11-d2m-libcxx' "$ROOT_DIR" 2>/dev/null; then
  fail "unpacked D2M libc++ reference found in init/property text"
fi
pass "unpacked D2M libc++ reference absent from init/property text"
...
[ ! -e "${ROOT_DIR}/init.recovery.wlan.rc" ] || fail "D2N ramdisk unexpectedly contains init.recovery.wlan.rc"
pass "D2N ramdisk does not contain init.recovery.wlan.rc"
if grep -R -Fq 'import /init.recovery.wlan.rc' "${ROOT_DIR}/init.recovery.qcom.rc" 2>/dev/null; then
  fail "D2N init.recovery.qcom.rc imports Wi-Fi lane"
fi
pass "D2N init.recovery.qcom.rc does not import Wi-Fi lane"

check_sha "${ROOT_DIR}/system/bin/keystore2" "$EXPECTED_KEYSTORE2_SHA256" "unpacked TWRP keystore2"
check_sha "${ROOT_DIR}/vendor/lib64/rm11-d2n-libcxx/libc++.so" "$EXPECTED_LIBCXX_SHA256" "unpacked scoped libc++"
check_sha "${ROOT_DIR}/vendor/lib64/rm11-d2m-libcxx/libc++.so" "$EXPECTED_LIBCXX_SHA256" "unpacked inherited D2M scoped libc++"
...
check_service_semantics "$ROOT_DIR" "unpacked D2N"
check_ui_defaults "$ROOT_DIR" "unpacked D2N"

if find "$ROOT_DIR" -name 'spss1*.mdt' -o -name 'spss1*.b*' | grep -q .; then
  fail "D2N ramdisk unexpectedly stages SPSS firmware"
fi
pass "D2N ramdisk does not stage SPSS firmware"
...
pass "crypto expectation is provable: D2N is outside RM11_NO_DECRYPT_PRODUCTS, so BoardConfig selects the TW_INCLUDE_CRYPTO lane"

printf '\n===== AVB FINGERPRINT =====\n'
grep -F 'com.android.build.recovery.fingerprint' "$tmp_avb" || true

printf '\nRESULT: PASS. D2N preflash gate passed. This script does not flash anything.\n'
